...
local AutoRefreshBox = Instance.new("TextButton")
AutoRefreshBox.BackgroundColor3 = Color3.fromRGB(70, 70, 78)
AutoRefreshBox.Text = "☐ 自动刷新"
AutoRefreshBox.TextColor3 = Color3.fromRGB(255, 255, 255)
AutoRefreshBox.Font = Enum.Font.SourceSansBold
AutoRefreshBox.Parent = BottomBar
...
StyleButton(AutoRefreshBox, Color3.fromRGB(70, 70, 78))
...
local function UpdateStatus(extra)
    local mode = AutoRefresh and "自动刷新中" or "自动刷新已关闭"
...
--====================================================
-- 兼容别名修复：防止不同版本变量名混用导致无法执行
--====================================================
local MinBtn = Minimize
local CloseBtn = Close
local Content = ContentFrame
local StatusLabel = Status
local Search = SearchBox
local Bottom = BottomBar
local RefreshBtn = Refresh
local CopyBtn = Copy
local FavBtn = FavoriteBtn
local ExportBtn = ExportLuaBtn
local ClearBtn = ClearAll
local Resize = ResizeHandle
local Layout = ListLayout

local Count = GetSectionCount
local Rebuild = RebuildSectionText
local UpdateSectionBtns = UpdateSectionButtons
local SetDisplay = SetDisplayText
local GetCurrentText = GetCurrentAllText
local StatusFunc = UpdateStatus
...
local function ExportLua(title, texts, fileName)
    ...
    if writefile then
        writefile(fileName or "UITextExport.lua", luaText)
        saved = true
    end
...
local function LayoutUI()
   ...
   local bottomButtons = {Refresh, AutoRefreshBox, Copy, BlockBtn, FavoriteBtn, ExportLuaBtn, ClearAll}
...
AutoRefreshBox.MouseButton1Click:Connect(function()
    AutoRefresh = not AutoRefresh

    if AutoRefresh then
        AutoRefreshBox.Text = "☑ 自动刷新"
        AutoRefreshBox.BackgroundColor3 = Color3.fromRGB(160, 120, 60)
    else
        AutoRefreshBox.Text = "☐ 自动刷新"
        AutoRefreshBox.BackgroundColor3 = Color3.fromRGB(70, 70, 78)
    end
