...
SetDisplay = function(text, autoBottom, animate, forceRebuild)
    ...
    if animate then
        local origPos = Scroll.Position
        Scroll.Position = UDim2.new(origPos.X.Scale, origPos.X.Offset + 16, origPos.Y.Scale, origPos.Y.Offset)
        Tween(Scroll, {Position = origPos}, 0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    end
...
local function UpdateStatus(msg)
    local autoStr = AutoRefreshEnabled and "自动刷新:开" or "自动刷新:关"
    local blockStr = BlockMode and "屏蔽:开" or "屏蔽:关"
    local searchStr = (CleanText(SearchBox.Text) ~= "") and "搜索中" or ""
    local base = autoStr.."｜"..blockStr
    if searchStr ~= "" then base = base.."｜"..searchStr end
    if msg and msg ~= "" then
        StatusLabel.Text = base.."｜"..CurrentSection.."｜"..Count(CurrentSection).."条｜"..msg
    else
        StatusLabel.Text = base.."｜"..CurrentSection.."｜"..Count(CurrentSection).."条"
    end
end
...
local function SearchNow()
    ...
    -- 直接清空并填充搜索结果列表（绕过 SetDisplay 的内容检查和 token 逻辑，确保移动端搜索列表一定能显示）
    ClearScroll()
    if #result == 0 then
        local empty = New("TextButton", {
            Size = UDim2.new(1, -12, 0, 34),
...
local function GetCircleTargetPosition()
    ...
    local x = math.clamp(ap.X + asz.X - 40, 10, math.max(10, vw - 64))
    local y = math.clamp(ap.Y + asz.Y - 40, 10, math.max(10, vh - 64))
