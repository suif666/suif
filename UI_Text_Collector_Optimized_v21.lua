-- Roblox UI 文本提取器 v21 优化版（大幅减少新增文本时的卡顿）
-- 主要优化：
-- 1. 引入行对象池（Row Pool）复用 UI 实例，减少 Instance.new / Destroy 开销
-- 2. 批量创建行时自动 yield（每12行 task.wait()），彻底解决主线程阻塞卡顿
-- 3. 强化增量追加逻辑 + 全量重建时也带 yield
-- 4. ClearScroll 改为回收而不是直接 Destroy
-- 功能保持不变：分区扫描 / 手动刷新 / 自动刷新 / 搜索 / 复制 / 收藏栏 / 导出Lua / 删除 / 屏蔽 / 缩放 / 最小化
...
-- ==================== 优化：行对象池 ====================
local RowPool = {}          -- 回收的行 Frame
local MAX_POOL_SIZE = 400   -- 最多缓存 400 个，避免内存无限增长
...
local Title = New("TextLabel", {BackgroundTransparency=1, Text="UI 文本提取器 v21 优化版", TextColor3=Color3.new(1,1,1), Font=Enum.Font.SourceSansBold, TextXAlignment=Enum.TextXAlignment.Left, TextTruncate=Enum.TextTruncate.AtEnd}, Main)
...
Corner(LeftPanel,8); Stroke(LeftPanel, Theme.Stroke,1,0.38)
...
local SectionFrame = New("Frame", {BackgroundColor3=Theme.Panel2, BorderSizePixel=0, ClipsDescendants=true}, LeftPanel)
Corner(SectionFrame,8); Stroke(SectionFrame, Theme.Stroke,1,0.38)

local SearchBox = New("TextBox", {Text="", PlaceholderText="搜索当前分区文本...", ClearTextOnFocus=false, BackgroundColor3=Theme.Card, TextColor3=Color3.new(1,1,1), PlaceholderColor3=Color3.fromRGB(155,155,160), Font=Enum.Font.SourceSans}, LeftPanel)
Corner(SearchBox,8); Stroke(SearchBox, Theme.Stroke,1,0.38)
local SearchBtn = New("TextButton", {Text="搜", TextColor3=Color3.new(1,1,1), Font=Enum.Font.SourceSansBold, BackgroundColor3=Theme.Purple}, LeftPanel)
...
-- 记录当前已渲染到列表里的行内容与对应的行实例
local DisplayedLines = {}
local DisplayedRows = {}

-- ==================== 优化后的 ClearScroll（使用对象池回收） ====================
...
local function CreateDisplayRow(line, index)
    ...
    local label = New("TextButton", {
        ...
        TextSize = compactRow and 11 or 13,
        Font = Enum.Font.Code,
    ...
    local add = New("TextButton", {
        ...
        Position = UDim2.new(1, -(actionW * 3 + actionGap * 2 + rightPad), 0.5, -actionH / 2),
        Text = compactRow and "藏" or "添加",
        TextColor3 = Color3.new(1, 1, 1),
        TextSize = compactRow and 10 or 12,
    ...
    local del = New("TextButton", {
        Size = UDim2.new(0, actionW, 0, actionH),
        Position = UDim2.new(1, -(actionW * 2 + actionGap + rightPad), 0.5, -actionH / 2),
        Text = compactRow and "删" or "删除",
        TextColor3 = Color3.new(1, 1, 1),
        TextSize = compactRow and 10 or 12,
        Font = Enum.Font.SourceSansBold,
    ...
    local copy = New("TextButton", {
        Size = UDim2.new(0, actionW, 0, actionH),
        Position = UDim2.new(1, -(actionW + rightPad), 0.5, -actionH / 2),
        Text = compactRow and "复" or "复制",
        TextColor3 = Color3.new(1, 1, 1),
        TextSize = compactRow and 10 or 12,
        Font = Enum.Font.SourceSansBold,
...
-- ==================== 优化后的 SetDisplay（批量创建 + yield，彻底解决卡顿） ====================
SetDisplay = function(text, autoBottom)
    ...
    -- 判断是否可以纯追加
    local canAppend = #newLines >= #DisplayedLines
    if canAppend then
        for i = 1, #DisplayedLines do
            if DisplayedLines[i] ~= newLines[i] then
                canAppend = false
                break
            end
        end
    ...
    if canAppend and #DisplayedRows > 0 then
        -- 纯追加场景：只创建新增的行（带 yield）
        for i = #DisplayedLines + 1, #newLines do
            local row = CreateDisplayRow(newLines[i], i)
            table.insert(DisplayedRows, row)
            if (i - #DisplayedLines) % 12 == 0 then
                task.wait()  -- 关键优化：防止创建大量行时卡顿
            end
        end
...
local function LayoutUI()
    ...
    local sectionBtnH = mobile and 22 or 26
    local actionH = mobile and 22 or 26
    ...
    LeftPanel.Size = UDim2.new(0, sideW, 0, availableLeftH)
    LeftPanel.Position = UDim2.new(0, pad, 0, pad)
    LeftPanel.ScrollBarThickness = mobile and 5 or 6
