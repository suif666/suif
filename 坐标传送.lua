...
-- ==================== 宿主集成 ====================
-- 主脚本（Suture Hub）提供 getgenv().Tabs.CoordTPTab 时：
--   界面直接建在主窗口的那个 Tab 里，不再自己开一个独立窗口
--   （避免两个窗口抢同一个 RightShift 开关）
-- 没有宿主时：老样子，自己开窗口（可单独执行）
local HostTab = (getgenv().Tabs and getgenv().Tabs.CoordTPTab)
    or getgenv().SutureCoordTPTab

if HostTab and getgenv().__SUTURE_COORDTP_LOADED then
    -- 已经建过界面了，不重复建
    pcall(function()
        WindUI:Notify({
            Title = "坐标传送",
            Content = "界面已经在「坐标传送」标签页里了",
            Icon = "map-pin",
            Duration = 3,
        })
    end)
...
if HostTab then
    ...
    tab = win:Tab({ Title = "坐标传送", Icon = "map-pin", Locked = false })
end
...
-- 标记已加载（集成模式下防止重复建界面）
if HostTab then
    getgenv().__SUTURE_COORDTP_LOADED = true
end

if HostTab then
    notify("坐标传送已加载", "在主窗口的「坐标传送」标签页里", "map-pin")
else
