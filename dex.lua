...
if syn and syn.protect_gui then syn.protect_gui(game:GetService("CoreGui")) end
local selection
local nodes = {}
		local oldgame = game
...
if scrollbar.Gui:FindFirstChild("Up") then
    scrollbar.Gui.Up.ZIndex = 3
end
if scrollbar.Gui:FindFirstChild("Down") then
	...
	Explorer.GuiElems = {}
		...
		context:Register("RENAME",{Name = "重命名", IconMap = Explorer.MiscIcons, Icon = "Rename", DisabledIcon = "Rename_Disabled", Shortcut = "F2", OnClick = function()
			...
			local plrRP = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
				...
				local OldAnimation
		context:Register("PLAY_TWEEN",{Name = "播放补间动画", IconMap = Explorer.MiscIcons, Icon = "Play", OnClick = function()
			local sList = selection.List
					for i = 1, #sList do
				local node = sList[i]
				local Obj = node.Obj
		...
		context:Register("UNBLOCK_REMOTE",{Name = "解除阻止", IconMap = Explorer.MiscIcons, Icon = "Play", DisabledIcon = "Empty", OnClick = function()
			...
			if #sList == 1 then
				...
				context:Register("3DVIEW_MODEL",{Name = "3D预览对象", IconMap = Explorer.LegacyClassIcons, Icon = 54, OnClick = function()
			local sList = selection.List
			local isa = game.IsA
		...
		context:Register("VIEW_OBJECT",{Name = "查看对象（右键点击重置）", IconMap = Explorer.LegacyClassIcons, Icon = 5, OnClick = function()
			local sList = selection.List
			local isa = game.IsA
					for i = 1,#sList do
				local node = sList[i]
		...
		context:Register("DUMP_FUNCTIONS",{Name = "转储函数", IconMap = Explorer.MiscIcons, Icon = "SelectChildren", DisabledIcon = "Empty", OnClick = function()
			...
			if scr then ScriptViewer.DumpFunctions(scr) end
				...
				context:Register("FIRE_TOUCHTRANSMITTER",{Name = "触发TouchTransmitter", OnClick = function()
			...
			for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("TouchTransmitter") then firetouchinterest(hrp, v.Obj.Parent, 0) end end
				...
				context:Register("FIRE_CLICKDETECTOR",{Name = "触发ClickDetector", OnClick = function()
			local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
			if not hrp then return end
			for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ClickDetector") then fireclickdetector(v.Obj) end end
		end})
				context:Register("FIRE_PROXIMITYPROMPT",{Name = "触发ProximityPrompt", OnClick = function()
			local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
			if not hrp then return end
	...
	Lib.IconMap = (function()
		...
		local funcs = {}
			...
			context:Clear()
				...
				context:Add({Name = "- 请选择 -", function() self.Selected = nil self.OnSelect:Fire(nil) self:Update() end})
			end
					for i = 1,#options do
...
local API,RMD,env,service,plr,create,createSimple 
	...
	ModelViewer.StopViewModel = function(updating)
		...
		if not updating then
			...
			pathLabel.Gui.Text = "位置: " .. getPath(originalModel)
			window:Show()
...
local API,RMD,env,service,plr,create,createSimple 
	...
	Properties.SoundPreviewProp = {Category = "Data", ValueType = {Name = "声音播放器"}, Class = "Sound", Name = "Preview", Tags = {}}
	...
	Properties.ShowExplorerProps = function()
			...
			while apiClass do
				...
				for name,val in pairs(attrs) do
					...
					if not foundAttrs[name] then
						...
						local attrProp = {IsAttribute = true, Name = "属性_"..name, AttributeName = name, DisplayName = name, Class = "Instance", ValueType = valType, Category = "Attributes", Tags = {}}
		...
		elseif typeName == "UDim" then
			...
			result[2] = makeSubProp(prop,".Offset",{Name = "整数"})
		elseif typeName == "UDim2" then
		...
		elseif typeName == "Faces" then
			result[1] = makeSubProp(prop,".Back",{Name = "布尔值"})
			result[2] = makeSubProp(prop,".Bottom",{Name = "布尔值"})
			result[3] = makeSubProp(prop,".Front",{Name = "布尔值"})
			result[4] = makeSubProp(prop,".Left",{Name = "布尔值"})
			result[5] = makeSubProp(prop,".Right",{Name = "布尔值"})
			result[6] = makeSubProp(prop,".Top",{Name = "布尔值"})
		elseif typeName == "Axes" then
			result[1] = makeSubProp(prop,".X",{Name = "布尔值"})
			result[2] = makeSubProp(prop,".Y",{Name = "布尔值"})
			result[3] = makeSubProp(prop,".Z",{Name = "布尔值"})
		end
		...
		Properties.EntryTemplate = create({
			...
			{3,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="PropName",Parent={2},Position=UDim2.new(0,2,0,0),Size=UDim2.new(1,-2,1,0),Text="瞄定",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=0.10000000149012,TextTruncate=1,TextXAlignment=0,}},
			...
			{5,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://5642383285",ImageRectOffset=Vector2.new(144,16),ImageRectSize=Vector2.new(16,16),Name="Icon",Parent={4},Position=UDim2.new(0,2,0,2),ScaleType=4,Size=UDim2.new(0,16,0,16),}},
			{6,"TextButton",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderSizePixel=0,Font=4,Name="ToggleAttributes",Parent={2},Position=UDim2.new(1,-85,0,0),Size=UDim2.new(0,85,0,22),Text="[设置: OFF]",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=0.10000000149012,Visible=false,}},
		...
		local guiItems = create({
			...
			{2,"Frame",{BackgroundColor3=Color3.new(0.20392157137394,0.20392157137394,0.20392157137394),BorderSizePixel=0,Name="ToolBar",Parent={1},Size=UDim2.new(1,0,0,22),}},
			{3,"Frame",{BackgroundColor3=Color3.new(0.14901961386204,0.14901961386204,0.14901961386204),BorderColor3=Color3.new(0.1176470592618,0.1176470592618,0.1176470592618),BorderSizePixel=0,Name="SearchFrame",Parent={2},Position=UDim2.new(0,3,0,1),Size=UDim2.new(1,-6,0,18),}},
			{4,"TextBox",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClearTextOnFocus=false,Font=3,Name="SearchBox",Parent={3},PlaceholderColor3=Color3.new(0.39215689897537,0.39215689897537,0.39215689897537),PlaceholderText="搜搜资源",Position=UDim2.new(0,4,0,0),Size=UDim2.new(1,-24,0,18),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=0,}},
			{5,"UICorner",{CornerRadius=UDim.new(0,2),Parent={3},}},
...
local API,RMD,env,service,plr,create,createSimple 
	...
	local fileName = "场所_"..game.PlaceId.."_"..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.."_{TIMESTAMP}"
	local Saving = false
...
local API,RMD,env,service,plr,create,createSimple 
	...
	SettingsWindow.Init = function()
		...
		local titleonmiddle = AddCheckbox("窗口标题居中", Settings.Window.TitleOnMiddle)
		titleonmiddle.OnInput:Connect(function()
			Settings.Window.TitleOnMiddle = titleonmiddle.Toggled
		end)
		
		local bgTransparency = AddTextbox("背景透明度", tostring(Settings.Window.Transparency), 15)
		bgTransparency.FocusLost:Connect(function()
			Settings.Window.Transparency = tonumber(bgTransparency.Text)
		end)
		
		local classIcon = AddDropdown("类图标", {"Old", "NewDark", "Vanilla3"}, Settings.ClassIcon, false, 100)
		...
		local showMoreInfo = AddCheckbox("显示反编译脚本信息", Settings.ScriptViewer.ShowMoreInfo)
		showMoreInfo.OnInput:Connect(function()
			Settings.ScriptViewer.ShowMoreInfo = showMoreInfo.Toggled
		end)
		
		AddSeperator("反编译器")
		AddText("如果执行器不支持反编译，将使用备用选项")
		AddText("必须使用 'getbytecode' 来启用回退反编译器")
		local decompilerOption = {"Konstant", "AdvancedDecompiler", "Shiny"}
		local decompiler = AddDropdown("反编译器备用选项", decompilerOption, Settings.Decompiler.DecompilerFallback, false, 125)
		decompiler.OnSelect:Connect(function()
			Settings.Decompiler.DecompilerFallback = decompiler.Selected
		end)
		
		local ShinyPort = AddTextbox("shiny反编译端口", tostring(Settings.Decompiler.ShinyDecompilerPort), 50)
		...
		local preferFallback = AddCheckbox("优先使用备用反编译器", Settings.Decompiler.PreferDecompilerFallback)
...
local plr = service.Players.LocalPlayer or service.Players.PlayerAdded:wait()
	...
	Main.ExportSettings = function()
		...
		local gui = create({
			...
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="超级无敌DEV工具[汉化:MIAO]",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
			{7,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="StatusText",Parent={3},Position=UDim2.new(0,20,0,110),Size=UDim2.new(0,180,0,25),Text="抓取API",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="原作者by Chillz|汉化:MIAO",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
		...
		Main.CreateApp({Name = "点击零件以选择", IconMap = Explorer.ClassIcons, Icon = "SelectionBox", OnClick = function(callback)
			if callback then
				local mouse = Main.Mouse
		...
		Main.CreateApp({Name = "3D查看器", IconMap = Main.LargeIcons, Icon = "Object", Window = ModelViewer.Window})
	...
	Main.Uninit = function()
		...
		for _, gui in pairs(Main.GetSecureContainer():GetChildren()) do
			if string.sub(gui.Name,1,5) == "_DPP_" then 
				gui:Destroy()
			end
		end
	end
